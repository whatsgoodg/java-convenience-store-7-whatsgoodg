...
public class NonPromotionCheckService {
    ...
    /**
     * 구매하고자 하는 상품들에 프로모션 재고가 부족하여 정가로 구매해야하는 상품 정보(이름, 수량)들을 반환 (구매수량) / (buy+get) - (재고수량) / (buy+get) 이 1 이상 이거나
     * (구매수량) / (buy+get) - (재고수량) / (buy+get) 이 0이면서 (구매수량 % (buy+get))이 1 이상이여야 함 이러한 경우 ((구매 수량) / (byt+get)) *
     * (buy+get) + 구매 수량 % (buy+get)를 정가로 구매해야함.
     *
     * @param purchaseProductInfos
     * @return 정가로 구매해야하는 상품 정보를 리스트로 반환, 빈 리스트 반환시 그러한 상품이 존재하지 않음
     */
    public List<NonPromotionalProductInfo> getNonPromotionalProductInfos(
            final List<PurchaseProductInfo> purchaseProductInfos) {
        List<NonPromotionalProductInfo> nonPromotionalProductInfos = new ArrayList<>();

        purchaseProductInfos.forEach(purchaseProductInfo -> {
            Optional<NonPromotionalProductInfo> fullPriceProductInfo = getNonPromotionalProductInfo(
                    purchaseProductInfo);
    ...
    private Optional<NonPromotionalProductInfo> getNonPromotionalProductInfo(
            final PurchaseProductInfo purchaseProductInfo) {
    ...
    private Optional<NonPromotionalProductInfo> calculateNonPromotionalProductInfo(final Product product,
                                                                                   final PurchaseProductInfo purchaseProductInfo) {
